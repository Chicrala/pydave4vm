...
execute_hybrid:
This version will export the images as text documents since the hardware
currently available for me can't hold a large database file.
...
def prepare(config_path, os_, downloaded=None):
    ...
    # Formatting the paths for the data.
    path_ = path.replace('/data/','/images/')
    imgs_path = {'Base':path_, 'U0':path_+'U0/', 'V0':path_+'V0/', 'W0':path_+'W0/', 'Bx':path_+'Bx/', 
    			 'By':path_+'By/', 'Bz':path_+'Bz/', 'Sn':path_+'Sn/', 'St':path_+'St/', 'Ss':path_+'Ss/'}

    # Creating the export path for the images.
    for item in imgs_path.keys():
	    if os.path.exists(imgs_path[item]) != True:
	    	os.makedirs(imgs_path[item])
